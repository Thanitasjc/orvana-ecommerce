$(window).on("load", function () {
    $(".tf-swiper").each(function (index, element) {
        ...
        if ($this.hasClass("sw-slide-show")) {
            preview = 1;
            tablet = 1;
            mobile = 1;
            mobileSm = 1;
...
if ($(".tf-sw-thumbs").length > 0) {
    $(".tf-sw-thumbs").each(function () {
        var $section = $(this);
        var thumbEl = $section.find(".sw-thumb")[0];
        var mainEl = $section.find(".sw-main-thumb")[0];

        if (!thumbEl || !mainEl) {
            return;
        }

        var slideCount = $section.find(".sw-main-thumb .swiper-slide").length;
        ...
        var $navNext = $section.find(".nav-next-swiper");
        var $navPrev = $section.find(".nav-prev-swiper");
        var $pagination = $section.find(".sw-pg-thumb");

        if ($navNext.length && $navPrev.length) {
            mainOptions.navigation = {
                nextEl: $navNext[0],
                prevEl: $navPrev[0],
            };
        }
        ...
        var mainSwiper = new Swiper(mainEl, mainOptions);

        mainSwiper.on("slideChangeTransitionEnd", function () {
            var index = enableLoop ? this.realIndex : this.activeIndex;
        ...
        if (enableLoop) {
            thumbSwiper.slideToLoop(mainSwiper.realIndex, 0);
        } else {
            thumbSwiper.slideTo(mainSwiper.activeIndex, 0);
        }
    });
}
